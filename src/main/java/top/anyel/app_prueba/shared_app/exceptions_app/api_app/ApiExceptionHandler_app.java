...
import static top.anyel.app_prueba.shared_app.exceptions_app.api_app.ApiErrorResponse_app.*;

@RestControllerAdvice
@Slf4j
class ApiExceptionHandler_app extends ResponseEntityExceptionHandler{
    ...
    protected ResponseEntity handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
                                                          HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        List<ApiError_app> errors = ex.getBindingResult().getFieldErrors()
                .stream()
                .map(fieldError -> ApiError_app.fieldApiError(fieldError.getField(), fieldError.getDefaultMessage(), fieldError.getRejectedValue()))
    ...
    ResponseEntity<ApiErrorResponse_app> handleConstraintViolationException(ConstraintViolationException exception) {
        List<ApiError_app> errors = exception.getConstraintViolations()
                .stream()
                .map(violation -> ApiError_app.fieldApiError(violation.getPropertyPath().toString(), violation.getMessage(), violation.getInvalidValue()))
                .collect(Collectors.toList());
        return unprocessableEntity(errors, "Validación de argumentos fallida");
    }

    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    ResponseEntity<ApiErrorResponse_app> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException exception) {
    ...
    ResponseEntity<ApiErrorResponse_app> handleThrowable(Throwable throwable) {
